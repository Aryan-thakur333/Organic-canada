...
import { resolveAdminCookieOptions } from "./src/lib/admin-cookie-options"
...
const redisPrefix = process.env.REDIS_PREFIX?.trim() || "eatsie"

if (redisTls && redisUrl && !redisUrl.toLowerCase().startsWith("rediss://")) {
  throw new Error("[medusa-config] REDIS_TLS=true requires a rediss:// REDIS_URL")
}
if (!/^[a-zA-Z0-9:_-]+$/.test(redisPrefix)) {
  throw new Error("[medusa-config] REDIS_PREFIX may contain only letters, numbers, colon, underscore, and hyphen")
}
...
const isLocalStable = process.env.EATSIE_LOCAL_STABLE === "true" &&
  process.env.EATSIE_RUNTIME_MODE === "local-stable" &&
  process.env.EATSIE_ALLOW_FAKE_REDIS === "true"

if (!skipEnvCheck) {
  const isRedisRequired = process.env.NODE_ENV === "production" && !isLocalStable
  const productionRequired = isRedisRequired ? ["REDIS_URL"] : []
  ...
  if (isLocalStable) {
    console.warn("\n══════════════════════════════════════════════════════")
    console.warn("  ⚠️  LOCAL ONLY: fake Redis is enabled. Do not use this mode in production.")
    console.warn("══════════════════════════════════════════════════════\n")
  }
...
export default defineConfig({
  projectConfig: {
    ...
    // `medusa start` uses production cookie defaults (Secure + SameSite=None).
    // Local stable Admin is intentionally served over http://localhost:9000,
    // where browsers reject Secure cookies and the login immediately falls
    // back to 401. Override only this explicitly local runtime.
    cookieOptions: resolveAdminCookieOptions({
  ...
  modules: {
    ...(redisUrl ? {
      ...
      [Modules.WORKFLOW_ENGINE]: {
        ...
        options: {
          redisUrl,
          queueName: `${redisPrefix}:workflows`,
          jobQueueName: `${redisPrefix}:workflow-jobs`,
          redisOptions: { connectTimeout: 5000 },
          workerOptions: { concurrency: 5 },
        },
      },
    ...
    payment: {
      ...
      options: {
        providers: [
          {
            resolve: "./src/modules/pos-cash-payment",
            id: "cash",
          },
          ...(stripeConfigured ? [{
    ...
    pricingManagement: {
      resolve: "./src/modules/pricing-management",
    },
    oms: {
      resolve: "./src/modules/oms",
      definition: {
        isQueryable: true,
      },
    },
    ...
    erp: {
      ...
      options: {
        enabled:
          process.env.ERP_ENABLED === "true",
        provider:
          process.env.ERP_PROVIDER || "odoo",
        baseUrl:
        ...
        timeoutMs:
          Number(process.env.ERP_TIMEOUT_MS || 10000),
      },
    },
  },
})